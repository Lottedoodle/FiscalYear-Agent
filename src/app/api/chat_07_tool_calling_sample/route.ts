...
export async function POST(req: NextRequest) {
  try {
    ...
    // แปลง UIMessage จาก Vercel AI SDK ให้เป็น LangChain messages
    const langchainMessages = messages.map((msg) => {
      // UIMessage มี parts array ที่เก็บ content
      let textContent = ''
    ...
    // เพิ่ม system message เข้าไปตอนต้น
    const messagesWithSystem = [
      new SystemMessage("You are a helpful and friendly AI assistant who speaks Thai."),
      ...langchainMessages,
    ]
    ...
    const aiResponse = await modelWithTools.invoke(messagesWithSystem)
