...
const Message = ({
  ...
  if (bubbleStyle) {
    if (isAssistant) {
      ...
      return (
        <div className={cn("group w-full", className)} {...props}>
          {children}
        </div>
      );
    } else {
      ...
      return (
        <div
          className={cn("group flex flex-col items-end", className)}
          {...props}
        >
          {children}
        </div>
...
const MessageActions = ({
  ...
  if (bubbleStyle) {
    ...
    classNames = cn(
      "text-muted-foreground flex items-center gap-2 relative z-10",
      className,
    );
  }
